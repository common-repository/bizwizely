<?php
/**
* Plugin Name: BizWizely - Upgrade WooCommerce Store to Android & iOS Mobile Apps
* Plugin URI: https://www.BizWizely.com/
* Description: Upgrade Your Woo-Commerce Store Into an E-Commerce App
* Version: 2.4
* Author: BizWizely
* Text Domain: bizwizely
* Author URI: http://bizwizely.com/
* Tested up to: 5.7.2
...
if ( ! defined( 'WPINC' ) ) {
	...
	global $wp_version;
if ( $wp_version <= 4.9 ) {
	function bizwizely_wo_incompatibility_with_wp_version() {
		?>
			<div class="notice notice-error">
				<p><?php esc_html_e( 'BizWizely requires that WordPress 4.9 or greater be used. Update to the latest WordPress version.', 'BizWizely' ); ?>
					<a href="<?php echo esc_url( admin_url( 'update-core.php' ) ); ?>"><?php esc_html_e( 'Update Now', 'BizWizely' ); ?></a></p>
			</div>
			<?php
	}

	add_action( 'admin_notices', 'bizwizely_wo_incompatibility_with_wp_version' );
}
	...
	add_action(
		...
		function () {
			register_rest_route(
				...
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_get_menu_items',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-menu-names',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_get_menu_names',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-verify-user',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_verify_user',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-profile-meta',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_get_profile_meta',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-order-payment-url',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_get_order_payment_url',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-order-total',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_get_order_total',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-send-password-reset-link',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_send_password_reset_link',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-applicable-shipping-method',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_applicable_shipping_method',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-product-search',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_product_search',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-verify-cart-items',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_verify_cart_items',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-categories',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_categories',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-post-categories',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_post_categories',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-checkout-fields',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_checkout_fields',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-wc-points-rewards-effective-discount',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_wc_points_rewards_effective_discount',
					'args'     => array(
						'line_items'  => array(
							'required'    => true,
							'type'        => 'array',
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-wc-points-rewards-settings',
				array(
					'methods'  => 'GET',
					'callback' => 'bizwizely_wc_points_rewards_settings',
				)
			);
		}
	);
	...
	add_action(
		...
		function () {
			register_rest_route(
				'wc/v3',
				'/bizwizely-change-password',
				array(
					'methods'  => 'POST',
					'callback' => 'bizwizely_change_password',
					'args' => array(
	...
	add_filter( 'user_has_cap', 'bizwizely_allow_payment_without_login', 10, 3 );
	function bizwizely_allow_payment_without_login( $allcaps, $caps, $args ) {
	 ...
	 */
	add_filter( 'woocommerce_get_settings_products', 'bizwizely_add_subtab_settings', 10, 2 );
	function bizwizely_add_subtab_settings( $settings ) {
		$current_section = get_option( 'woocommerce_default_catalog_orderby' );

		if ( isset( $current_section ) ) {
			$settings[] = array(
				'name'     => __( 'bizwizely WC Default Catalog Orderby Settings', 'woocommerce' ),
	...
	register_rest_field(
		...
		'featured_image_src',
		array(
			'get_callback'    => 'bizwizely_get_image_src',
			'update_callback' => null,
			'schema'          => null,
		)
	);

	function bizwizely_get_image_src( $object, $field_name, $request ) {
	...
	register_rest_field(
		...
		'blog_images',
		array(
			'get_callback'    => 'bizwizely_get_images_urls',
			'update_callback' => null,
			'schema'          => null,
		)
	);

	function bizwizely_get_images_urls( $object, $field_name, $request ) {
	...
	register_rest_field(
		'product',
		'bizwizely_default_variation_id',
		array(
			'get_callback'    => 'bizwizely_get_default_variant',
			'update_callback' => null,
			'schema'          => null,
		)
	);
	function bizwizely_get_default_variant( $object, $field_name, $request ) {
	...
	register_rest_field(
		'product',
		'bizwizely_product_discount_percentage',
		array(
			'get_callback'    => 'bizwizely_get_product_discount_percentage',
			'update_callback' => null,
			'schema'          => null,
		)
	);
	function bizwizely_get_product_discount_percentage( $object, $field_name, $request ) {
	...
	function bizwizely_prepare_product_images( $response, $post, $request ) {
	...
	add_filter( 'woocommerce_rest_prepare_product_object', 'bizwizely_prepare_product_images', 10, 3 );
	...
	add_filter( 'woocommerce_rest_prepare_shop_order_object', 'bizwizely_rest_apply_coupon', 10, 3 );

	function bizwizely_rest_apply_coupon( $response, $object, $request ) {
		...
		if ( in_array(
			...
			$wc_points_rewards_discount_amount = $order->get_meta( '_bizwizely_wc_points_redeemed' );
			$customer_id                       = $request->get_param( 'customer_id' );
			$line_items                        = $request->get_param( 'line_items' );
			$reward_amount                     = bizwizely_get_points_rewards_discount_amount( $customer_id, $wc_points_rewards_discount_amount, $line_items ); // to be calculated
			...
			if ( empty( $already_redeemed ) && $reward_amount > 0 ) {
				...
				update_post_meta( $response->data['id'], '_bizwizely_wc_points_redeemed', $actual_points_to_redeemed );
				update_post_meta( $response->data['id'], '_bizwizely_wc_points_rewards_discount_code', $reward_coupon_code );
				update_post_meta( $response->data['id'], '_bizwizely_wc_points_rewards_discount_amount', $reward_amount );
		...
		if ( ! empty( $request->get_param( 'coupon_lines' ) ) ) {
			foreach ( $request->get_param( 'coupon_lines' ) as $item ) {
				...
				$bizwizely_order_checkout_url                       = ( $value = esc_url( $object->get_checkout_payment_url() ) ) ? $value : '';
				$response->data['order_checkout_payment_url'] = html_entity_decode( $bizwizely_order_checkout_url );
		...
		foreach ( $response->data['line_items'] as $key => $lineItem ) {
			...
			$response->data['line_items'][ $key ]['bizwizely_order_thumbnail'] = $medium_url[0];
		}
	...
	function bizwizely_get_menu_names() {
	...
	function bizwizely_get_menu_items( WP_REST_Request $request ) {
	...
	function bizwizely_categories() {
		...
		$category_tree = bizwizely_build_category_tree( $array_product_categories_items, 'parent', 'term_id' );
		return rest_ensure_response( $category_tree );

	}

	function bizwizely_post_categories() {
	...
	function bizwizely_verify_cart_items( WP_REST_Request $request ) {

		$parbizwizely   = $request->get_parbizwizely();
		$validate = bizwizely_basic_validate( $parbizwizely, array( 'line_items' ) );
		if ( $validate != true ) {
			return $validate;
		}

		$line_items = $parbizwizely['line_items'];
		$result     = array();
	 ...
	 ******/
	function bizwizely_product_search( WP_REST_Request $request ) {

		$param        = $request->get_parbizwizely();
		...
		$request->set_query_parbizwizely( $parameters );
	...
	function bizwizely_login( WP_REST_Request $request ) {

		$req = $request->get_json_parbizwizely();

		$validate = bizwizely_basic_validate( $req, array( 'username', 'password' ) );
		if ( $validate != true ) {
			return $validate;
		}
		...
		if ( isset( $user->errors ) ) {
			$error_message = strip_tags( bizwizely_convert_error_to_string( $user->errors ) );
			$error         = new WP_Error();
	 ...
	 * Verify the user.
	 ******/
	function bizwizely_verify_user( WP_REST_Request $request ) {

		$req = $request->get_json_parbizwizely();

		$validate = bizwizely_basic_validate( $req, array( 'username' ) );
		if ( $validate != true ) {
			return $validate;
		}
	...
	function bizwizely_get_profile_meta( WP_REST_Request $request ) {

		if ( isset( $request['id'] ) ) {
			$user_id = sanitize_text_field( $request['id'] );
		}
		$validate = bizwizely_basic_validate( $req, array( 'id' ) );
		if ( $validate != true ) {
			return $validate;
		}
		 ...
		 ******/
	function bizwizely_get_order_payment_url( WP_REST_Request $request ) {

		$req      = $request->get_json_parbizwizely();
		$validate = bizwizely_basic_validate( $req, array( 'order_id' ) );
		if ( $validate != true ) {
			return $validate;
		}
	...
	function bizwizely_send_password_reset_link( WP_REST_Request $request ) {
			$req      = $request->get_json_parbizwizely();
			$validate = bizwizely_basic_validate( $req, array( 'email' ) );
		if ( $validate != true ) {
			return $validate;
		}
	...
	function bizwizely_applicable_shipping_method( WP_REST_Request $request ) {

		$req      = $request->get_json_parbizwizely();
		$validate = bizwizely_basic_validate( $req, array( 'shipping', 'line_items' ) );
		if ( $validate != true ) {
			return $validate;
		}
		...
		foreach ( $shippingZones as $key => $shippingZone ) {
			...
			if ( ! empty( $shippingZones[ $key ]['locations'] ) ) {
				...
				foreach ( $shippingZones[ $key ]['locations'] as $locationkey => $locations ) {
					...
					} elseif ( ( ! $does_zone_contain_location_of_type_postcode ) && ( $shippingZones[ $key ]['locations'][ $locationkey ]['type'] == 'continent' ) && ( $shippingZones[ $key ]['locations'][ $locationkey ]['code'] == bizwizely_get_country_continent( $countryCode ) ) ) {
						$shippingZoneId = $shippingZones[ $key ]['id'];
	...
	function bizwizely_checkout_fields() {
		if ( in_array(
			'woocommerce-checkout-field-editor/woocommerce-checkout-field-editor.php',
			apply_filters( 'active_plugins', get_option( 'active_plugins' ) )
		)
		) {
				$result['checkout_shipping_fields']   = bizwizely_object_to_array( get_option( 'wc_fields_shipping', array() ) );
				$result['checkout_billing_fields']    = bizwizely_object_to_array( get_option( 'wc_fields_billing', array() ) );
				$result['checkout_additional_fields'] = bizwizely_object_to_array( get_option( 'wc_fields_additional', array() ) );
	...
	function bizwizely_wc_points_rewards_effective_discount( WP_REST_Request $request ) {
		if ( in_array(
			'woocommerce-points-and-rewards/woocommerce-points-and-rewards.php',
			apply_filters( 'active_plugins', get_option( 'active_plugins' ) )
		)
			...
			$parbizwizely                            = $request->get_parbizwizely();
			$line_items                        = $parbizwizely['line_items'];
			$customer_id                       = $parbizwizely['customer_id'];
			$wc_points_rewards_discount_amount = $parbizwizely['wc_points_rewards_discount_amount'];
			...
			if ( false !== strpos( $max_discount, '%' ) ) {
				$max_discount = bizwizely_bizwizely_calculate_discount_modifier( $max_discount );
			}
	...
	function bizwizely_wc_points_rewards_settings() {
		if ( in_array(
			'woocommerce-points-and-rewards/woocommerce-points-and-rewards.php',
			apply_filters( 'active_plugins', get_option( 'active_plugins' ) )
		)
	...
	add_filter( 'woocommerce_get_shop_coupon_data', 'bizwizely_create_virtual_coupon', 10, 2 );

	function bizwizely_create_virtual_coupon( $false, $data ) {
		...
		if ( is_admin() ) {
			...
			$coupon_amount     = 0;
		if ( bizwizely_is_coupon_code_valid( $data ) ) {
	...
	add_action( 'rest_api_init', 'bizwizely_create_api_customer_field' );

	function bizwizely_create_api_customer_field() {

		// register_rest_field ( 'name-of-post-type', 'name-of-field-to-return', array-of-callbacks-and-schema() )
		register_rest_field(
			'customer',
			'bizwizely-rewards-points-balance',
			array(
				'get_callback' => 'bizwizely_get_rewards_points_balance',
				'schema'       => null,
			)
		);
	}

	function bizwizely_get_rewards_points_balance( $object ) {
		if ( in_array(
			'woocommerce-points-and-rewards/woocommerce-points-and-rewards.php',
			apply_filters( 'active_plugins', get_option( 'active_plugins' ) )
		)
	...
	add_action( 'woocommerce_order_status_processing', 'bizwizely_order_processing' );
	function bizwizely_order_processing( $order_id ) {
		...
		if ( in_array(
			...
			$discount_code = $order->get_meta( '_bizwizely_wc_points_rewards_discount_code' );
			...
			if ( ! empty( $logged_redemption ) ) {
				...
				$points_redeemed = $order->get_meta( '_bizwizely_wc_points_redeemed' );
				// bail if bizwizely is not involved
				if ( ! $points_redeemed ) {
					return;
				}
				// Get amount of discount
				$discount_amount = $order->get_meta( '_bizwizely_wc_points_rewards_discount_amount' );
			...
			$order->add_order_note( sprintf( __( '%1$d %2$s redeemed for a %3$s discount.', 'woocommerce-points-and-rewards' ), $points_redeemed, bizwizely_get_points_label( $points_redeemed ), wc_price( $discount_amount ) ) );

		}

	}
	function bizwizely_change_password(WP_REST_Request $request){
		...
		if ( isset( $x->errors ) ) {
			$error_message = strip_tags( bizwizely_convert_error_to_string( $x->errors ) );
			$error         = new WP_Error();
			$error->add( 'message', __( $error_message . '' ) );
			return $error;
		}		
		...
		{	 		
			...
			{
				return new WP_Error( 'bizwizely_error', 'Sorry, your new password and old password can not be the same.', array( 'status' => 422 ) );
			}
			if($new_password == $confirm_password)
			{
				...
				if($uid) 
				{
					...
					return new WP_Error( 'bizwizely_error', 'Sorry, your account was not updated. Please try again later.', array( 'status' => 422 ) );
				}
			}
			else
			{					
				return new WP_Error( 'bizwizely_error', 'Sorry, both your passwords do not match. Please try again. ', array( 'status' => 422 ) );				
			}			
		} 
		else 
		{			
			return new WP_Error( 'bizwizely_error', 'Sorry, your old password is not correct. Please try again.', array( 'status' => 422 ) );
		}		
	...
	// //
	function bizwizely_get_points_rewards_discount_amount( $customer_id, $wc_points_rewards_discount_amount, $line_items ) {
		...
		if ( false !== strpos( $max_discount, '%' ) ) {
			$max_discount = bizwizely_calculate_discount_modifier( $max_discount );
			WC()->cart->empty_cart();
		}
			$max_discount = filter_var( $max_discount, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
	...
	function bizwizely_get_points_label( $count ) {

		list( $singular, $plural ) = explode( ':', get_option( 'wc_points_rewards_points_label' ) );

		return 1 == $count ? $singular : $plural;
	}
	function bizwizely_is_coupon_code_valid( $coupon_code ) {
	...
	function bizwizely_calculate_discount_modifier( $percentage ) {
	...
	function bizwizely_object_to_array( $data ) {
	...
	function bizwizely_build_category_tree( $flat, $pidKey, $idKey = null ) {
		$grouped = array();
		foreach ( $flat as $sub ) {
	...
	function bizwizely_convert_error_to_string( $er ) {
		 $string = ' ';
	...
	function bizwizely_basic_validate( $request, $keys ) {
	...
	function bizwizely_get_country_continent( $country ) {
		$data = array(
			...
			'AF' => array( 'AO', 'BF', 'BI', 'BJ', 'bizwizely', 'CD', 'CF', 'CG', 'CI', 'CM', 'CV', 'DJ', 'DZ', 'EG', 'EH', 'ER', 'ET', 'GA', 'GH', 'GM', 'GN', 'GQ', 'GW', 'YT', 'KE', 'KM', 'LY', 'LR', 'LS', 'MA', 'MG', 'ML', 'MR', 'MU', 'MW', 'MZ', 'NA', 'NE', 'NG', 'RE', 'RW', 'SC', 'SD', 'SH', 'SL', 'SN', 'SO', 'ST', 'SZ', 'TD', 'TG', 'TN', 'TZ', 'UG', 'ZA', 'ZM', 'ZW' ),
		);
	...
	add_action('init','woocomm_add_to_cart');	
function woocomm_add_to_cart() {

if($_GET['addcart']==1){
	
$contentall = trim(file_get_contents("php://input"));
	
$content=json_decode($contentall)->cartdata;
	
$jsonData = utf8_decode(base64_decode($content));
	
$param = (array)json_decode(preg_replace('/[\x00-\x1F\x80-\xFF]/', '',$jsonData),true);	
...
if($param['line_items']){
	unset($notexist);
    foreach($param['line_items'] as $prod) {
		if ( FALSE === get_post_status( $prod['product_id'] ) ) {
			//ho  $prod['product_id'] ;
			$notexist[]=$prod['product_id'];
			
			}else{
		if($prod['variation_id']){
			$membershipProduct = new WC_Product_Variable($prod['product_id']);
			$theMemberships = $membershipProduct->get_available_variations();
			unset($variationsArray);
			foreach ($theMemberships as $membership) {
				if ($prod['variation_id'] == $membership['variation_id']) {
					$variationID = $membership['variation_id'];
					$variationsArray[] = $membership['attributes'];
				}
			}
			WC()->cart->add_to_cart( $prod['product_id'], $prod['quantity'], $prod['variation_id'], $variationsArray );
		}else{
			$variationids="";
			$variationarray="";
			WC()->cart->add_to_cart( $prod['product_id'], $prod['quantity'],$variationids,$variationarray);
		}
		}
    }
}
...
$userdata=site_url().'/checkout/?token='.$user_id.'';
$allcartproduct=WC()->cart->get_cart();
	
	if($notexist !=''){
		$response = [
		'status' => true,
		'message' => 'Products successfully added to cart',
		'redirect_url'=>$userdata,
		'Product_not_exist'=>implode(',', $notexist),
		'all_cart_data'=>$allcartproduct,
	];
	}else{
	$response = [
		'status' => true,
...
add_action('init','login_at_checkout');
function login_at_checkout()
{
	if(isset($_GET['token'])){
		$user_id=$_GET['token'];
		//$user_id = $user_id;
